document.addEventListener('DOMContentLoaded', () => {
  ...
  const defaultBlogPosts = [
    ...
    {
      id: '3',
      title: 'Gastronomía con Sentido: El renacer de los sabores andinos',
      subtitle: 'Descubre cómo la cocina local en Cusco conecta la tierra, las comunidades campesinas y tu paladar.',
      author: 'Mónica Carrión',
      date: '2026-06-15',
      publishDate: '2026-06-15T12:00',
      image: 'assets/images/img-nosotros2.jpeg',
      content: '<h3>Sabores que cuentan historias</h3><p>La cocina peruana es famosa en todo el mundo, pero en las tierras altas de Cusco, la comida tiene un significado aún más profundo: es un puente directo con la Pachamama (Madre Tierra) y con las comunidades que la cultivan con amor y respeto ancestral.</p><p>En Rimay Viajes creemos que alimentar el cuerpo es también alimentar el espíritu. Por eso, en nuestras experiencias gastronómicas invitamos a los viajeros a participar en la preparación de una tradicional <strong>Pachamanca</strong>, cocida bajo tierra con piedras calientes, o a degustar variedades únicas de papas nativas, quinua orgánica y hierbas aromáticas locales cosechadas el mismo día. Cada bocado no solo es una explosión de sabor, sino también un acto de apoyo al comercio justo y al desarrollo sostenible de las comunidades andinas.</p>'
    }
  ];
      ...
      .then(snapshot => {
        if (snapshot.empty) {
          ...
          snapshot.forEach(doc => {
            docIdsInDb.push(doc.id);
            cachedBlogPosts.push(doc.data());
          });

          // Seed missing sample post 3 automatically if it's not present
          if (!docIdsInDb.includes('3')) {
            console.log("Seeding missing default sample post 3...");
            const post3 = defaultBlogPosts.find(p => p.id === '3');
            db.collection('posts').doc('3').set(post3)
              .then(() => {
                cachedBlogPosts.push(post3);
                renderBlogGrid();
                checkUrlQueryParam();
              })
              .catch(err => {
                console.error("Error seeding post 3:", err);
                renderBlogGrid();
                checkUrlQueryParam();
              });
          } else {
